...
export const VerificationPage = () => {
  ...
  const { control, setValue, handleSubmit, inputRefs, isFormValid, seconds, resendAvailable, codeError, errorVisible, handleInputChange, handleKeyDown, handleResendCode, validateCode, submitVerificationCode } = useVerificationCode(phone_number)
  ...
  return (
    <div className={clases.l_verificationPage}>
      ...
      <p className={clases.enterCode} dangerouslySetInnerHTML={{ __html: t('verificationPage.enterCode') }} />
      <form className={clases.form} onSubmit={handleSubmit(onSubmit)}>
        <CodeInputBox control={control} setValue={setValue} inputRefs={inputRefs} handleInputChange={handleInputChange} handleKeyDown={handleKeyDown} errorVisible={errorVisible} codeError={codeError} resendAvailable={resendAvailable} handleResendCode={handleResendCode} seconds={seconds} />
